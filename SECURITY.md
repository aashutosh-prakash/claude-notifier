...
- **Shell-free subprocess spawning.** Every child process — the runner and `osascript` — is spawned with `execFile`/`execFileSync` and an `argv` array, never through a shell. No command string is built by interpolation and handed to `/bin/sh`, so an attacker-influenced working-directory name or sound filename cannot inject commands. See `fireNotification` in `bin/install.js`.
- **Allowlisted env forwarding.** When `--test` runs the runner, only claude-nudge's own keys (`CLAUDE_NUDGE_SOUND`, `CLAUDE_NUDGE_STOP`) are forwarded from `settings.json` to the child — loader variables such as `DYLD_INSERT_LIBRARIES` / `LD_PRELOAD` are never passed through. See `settingsEnvStrings` in `bin/install.js`.
